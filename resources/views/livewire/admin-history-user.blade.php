                ...
                @if ($data->count() > 0)
                    ...
                    @forelse ($data as  $item)
                        <tr class="odd:bg-white even:bg-gray-100 border-b border-gray-200">
                            ...
                            <td
                                class="px-6   py-3 flex justify-center text-center text-gray-900 font-normal gap-1 ">

                               
                                    <button @click="$dispatch('open-modal2')" type="button"
                                        wire:click="showHistory({{ $item->user->id }})"
                                        class="flex items-center justify-center border-1 bg-blue-500 text-white cursor-pointer rounded-md px-2 py-1.5 hover:brightness-95 hover:scale-105 transition duration-150 ease-in-out">
                                        Detail
                                    </button>
                                    ...
                                    @if ($history)
                                        ...
                                        @forelse ($history as  $item)
                                            <tr class="odd:bg-white  even:bg-gray-100 border-b border-gray-200">
                                                ...
                                                <td
                                                    class="px-2 border-1 w-auto text-center border-slate-300 py-3  text-gray-900 font-normal ">
                                                    @if ($item->status == 'borrowed')
                                                        <div
                                                            class="inline-block text-xs px-2 text-red-600 bg-red-100 outline-1 outline-red-600 rounded-full">
                                                            Dipinjam
                                                        </div>
                                                    @elseif($item->status == 'returned')
                                                        <div
                                                            class="inline-block text-xs px-2 text-green-600 bg-green-100 outline-1 outline-green-600 rounded-full">
                                                            Dikembalikan
                                                        </div>
                                                    @endif
                                                </td>
